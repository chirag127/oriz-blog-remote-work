...
/**
 * Content desks — collapse raw category strings into the five desks this
 * log actually covers. Drives the desk marker colour (--v-*) on rows and
 * kickers so Focus / Money / Async / Life / Clients read differently at a
 * glance. (The underlying vertical keys stay dev/finance/culture/health/
 * personal so the shared --v-* token slots don't need renaming.)
 *
 *   Focus   (dev)      — the room, the desk, tools, deep work
 *   Money   (finance)  — invoicing, rates, taxes, getting paid
 *   Async   (culture)  — communication, handoffs, meetings, process
 *   Life    (health)   — boundaries, burnout, health away from an office
 *   Clients (personal) — finding, pitching, and keeping clients
 */
...
export function verticalFor(category: string): Vertical {
  ...
  if (/(life|health|burnout|boundar|wellbeing|balance|routine)/.test(c)) return 'health'
  if (/(money|invoic|rate|tax|pricing|paid|payment|finance|gst)/.test(c)) return 'finance'
  if (/(async|communicat|handoff|meeting|process|workflow|collab|remote)/.test(c)) return 'culture'
  if (/(focus|workspace|desk|office|tool|setup|deep work|productiv)/.test(c)) return 'dev'
  return 'personal'
}
